...
Terraformer also skips native runtime or controller-generated APIs such as `ResourceSlice`, `PodSchedulingContext`, `IPAddress`, `PodGroup`, `ControllerRevision`, `LeaseCandidate`, `CSINode`, `CSIStorageCapacity`, and `VolumeAttachment`, even when an older served version is not recognized by the pinned typed client.
