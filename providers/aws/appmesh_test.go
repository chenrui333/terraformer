...
func TestAppMeshFilterGatesMeshAndChildDiscovery(t *testing.T) {
	...
	tests := []struct {
		...
		{
			name: "typed mesh non-id filter does not block typed child non-id discovery",
			filters: []terraformutils.ResourceFilter{
		...
		{
			name: "untyped mesh id filter keeps opaque child UID discovery possible",
			filters: []terraformutils.ResourceFilter{
		...
		{
			name: "untyped child UID filter reaches child discovery",
			filters: []terraformutils.ResourceFilter{
				{FieldPath: "id", AcceptableValues: []string{"uid-node"}},
			},
			loadMeshes:            true,
			loadChildren:          true,
			loadOtherChildren:     true,
